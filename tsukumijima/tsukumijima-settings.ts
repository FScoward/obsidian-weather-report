...
export class Tsukumijima {
	...
	private requestUrl(city: City) {
		return (
			"https://weather.tsukumijima.net/api/forecast?city=" + city.cityCode
		);
	}
	...
	async getTempText(city: City): Promise<string> {
		return this.getWeatherData(city).then((weatherJson) => {
			const tempText =
				city.cityTitle +
				"の最高気温は" +
				weatherJson.forecasts[0].temperature.max.celsius +
				"度、最低気温は" +
				weatherJson.forecasts[0].temperature.min.celsius +
				"度です。" +
				"\n" +
				weatherJson.forecasts[0].detail.weather;
